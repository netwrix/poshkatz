...
$script:someCommands = @('privilege::debug', 'sekurlsa::logonpasswords', 'sekurlsa::tickets', 'sekurlsa::pth',  'kerberos::list', 'kerberos::ptt', 'kerberos::golden',
...
function GitTabExpansionInternal($lastBlock, $GitStatus = $null) {
    ...
    if ($lastBlock -match "^$(Get-AliasPattern mimikatz) (?<cmd>\S+)(?<args> .*)$") {
...
function TabExpansion($line, $lastWord) {
    ...
    switch -regex ($lastBlock) {
        ...
        "^$(Get-AliasPattern mimikatz) (.*)" { Expand-GitCommand $lastBlock }
